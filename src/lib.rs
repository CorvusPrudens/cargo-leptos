...
use ext::{color_eyre::CustomWrapErr, fs, Paint};
...
pub async fn run(args: Cli) -> Result<()> {
    ...
    let config =
        Config::load(opts, &cwd, &manifest_path, watch, bin_args, build_targets).dot_anyhow()?;
    ...
    match args.command {
        ...
        Serve(_) => command::serve(&config.current_project().wrap_anyhow_err("Serve error")?).await,
        Test(_) => command::test_all(&config).await,
        EndToEnd(_) => command::end2end_all(&config).await,
        Watch(_) => command::watch(&config.current_project().wrap_anyhow_err("Watch error")?).await,
